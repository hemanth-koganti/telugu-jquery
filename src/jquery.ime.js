( function ( $ ) {
	...
	IME.prototype = {
		...
		transliterate: function ( input, context, altGr ) {
			var patterns, regex, rule, replacement, i, retval, j, k, l;
			var temp3;
			var consopatterns = [];
			var consonants = ["k","K","i","I","5",";",":","p","P","\\}","\\'","\"","\\[","\\{","C","l","L","o","O","v","h","H","y","Y","c","/","j","J","n","N","b","M","\\<","m","u"];
			var vowelPatterns = [];
			var vowels = ["d", "e", "f", "r", "g", "t", "z", "s", "w", "a", "q", "\\=", "\\-"];
			var polluPatterns = [];
			...
			for ( j = 0; j < patterns.length; j++ ) {
				for ( k = 0; k < consonants.length; k++ ){
					if ( patterns[j][0] === consonants[k] ){
						consopatterns.push(patterns[j]);
						// debug("true");
						// debug(patterns[j]);
					}
				}
			...
			for ( i = 0; i < patterns.length; i++ ) {
				...
				// if ( input[0] === rule[1]){
				// 	debug("trueueueueeueueue");
				// }

				// Input string match test
				if ( regex.test( input ) ) {
					debug("inside transliterate for");
					debug(regex);
					debug(input);
					debug(rule);
					debug(replacement);
		...
		keypress: function ( e ) {
			var altGr = false,
				c, input, replacement,temp,temp1;
			debug("hello");
			debug(e);
			if ( !this.active ) {
				return true;
			...
			debug("replacement after return");
			debug(replacement);
